...
export function CustomerDetailModal({ customer, onClose }: CustomerDetailModalProps) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'credit': return 'bg-blue-100 text-primary-800';
        ...
        <div className="p-8 overflow-y-auto max-h-[calc(90dvh-200px)]">
          {activeTab === 'details' && (
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="bg-gradient-to-r from-success-500 to-success-600 p-6 rounded-2xl text-white">
                  <div className="flex items-center justify-between">
                    <div>
                    ...
                    <ShoppingBag className="h-8 w-8 text-success-200" />
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-r from-primary-500 to-primary-600 p-6 rounded-2xl text-white">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-primary-100 text-sm font-medium">Transactions</p>
                ...
                <div className="bg-gradient-to-r from-accent-500 to-accent-600 p-6 rounded-2xl text-white">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-accent-100 text-sm font-medium">Avg. Transaction</p>
                      <p className="text-2xl font-bold">{DEFAULT_CURRENCY} {averageTransaction.toFixed(2)}</p>
                    </div>
                    <CreditCard className="h-8 w-8 text-accent-200" />
                  </div>
                </div>

                <div className="bg-gradient-to-r from-warning-500 to-warning-600 p-6 rounded-2xl text-white">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-warning-100 text-sm font-medium">Credit Available</p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-secondary-50 rounded-2xl p-6">
                  <h3 className="text-lg font-bold text-secondary-900 mb-6 font-fraunces">Contact Information</h3>
                ...
                <div className="bg-secondary-50 rounded-2xl p-6">
                  <h3 className="text-lg font-bold text-secondary-900 mb-6 font-fraunces">Account Information</h3>
                  <div className="space-y-4">
                    <div className="flex items-center space-x-3">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-secondary-900">{format(new Date(customer.createdAt), 'MMM dd, yyyy')}</p>
                      </div>
                    </div>
                    <div className="flex items-center space-x-3">
                      <Calendar className="h-5 w-5 text-secondary-400" />
                      <div>
                        <p className="text-sm font-medium text-secondary-600">Last Purchase</p>
              ...
              {/* Credit Information */}
              <div className="bg-gradient-to-r from-primary-50 to-indigo-50 rounded-2xl p-6 border border-primary-200">
                <h3 className="text-lg font-bold text-secondary-900 mb-4 font-fraunces">Credit Information</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold text-success-600">{DEFAULT_CURRENCY} {creditAvailable.toFixed(2)}</p>
                  </div>
                </div>
                <div className="mt-4">
          ...
          {activeTab === 'transactions' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-bold text-secondary-900 font-fraunces">Transaction History</h3>
                <p className="text-secondary-600">{totalTransactions} transactions</p>
              </div>

              {customerTransactions.length === 0 ? (
                <div className="text-center py-12">
                  <Receipt className="h-16 w-16 text-secondary-300 mx-auto mb-4" />
                  <p className="text-secondary-500 text-lg">No transactions found</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {customerTransactions.map((transaction) => (
                    <div key={transaction.id} className="bg-white border border-secondary-200 rounded-2xl p-6 hover:shadow-md transition-shadow">
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          <h4 className="font-bold text-secondary-900">#{transaction.receiptNumber}</h4>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        <div>
                          <p className="text-secondary-600">Items</p>
                          <p className="font-medium">{transaction.items.length} items</p>
                        </div>
                        <div>
                          <p className="text-secondary-600">Payment Method</p>
                          <p className="font-medium capitalize">{transaction.paymentMethod}</p>
                        </div>
                        <div>
                          <p className="text-secondary-600">Cashier</p>
                      ...
                      {transaction.items.length > 0 && (
                        <div className="mt-4 pt-4 border-t border-secondary-200">
                          <p className="text-sm font-medium text-secondary-600 mb-2">Items:</p>
