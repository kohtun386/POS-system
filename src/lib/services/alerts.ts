...
export const alertHistoryService = {
  async getAll(limit: number = 100): Promise<AlertHistory[]> {
    ...
    return data.map(history => ({
      ...
      currentStock: history.current_stock ?? undefined,
      minStock: history.min_stock ?? undefined,
...
export const notificationServiceConfigService = {
  async getAll(): Promise<NotificationServiceConfig[]> {
    ...
    return data.map(config => ({
      ...
      configData: (config.config_data as Record<string, string | number | boolean>) ?? {},
  ...
  async getById(id: string): Promise<NotificationServiceConfig> {
    ...
    return {
      ...
      configData: (data.config_data as Record<string, string | number | boolean>) ?? undefined,
      isActive: data.is_active ?? true,
