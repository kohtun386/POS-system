...
function mapShopRow(row: Record<string, unknown>): Shop {
  return {
    ...
    businessType: ((row.business_type as string) || 'coffee_shop') as 'coffee_shop',
    ...
    subscriptionTier: ((row.subscription_tier as string) || 'free') as 'free' | 'growth' | 'pro',
    dailyOrderLimit: (row.daily_order_limit as number) ?? undefined,
    receiptSetting: ((row.receipt_setting as string) || 'ask') as 'always' | 'ask' | 'never',
...
export const featureDefinitionsService = {
  async getAll(): Promise<FeatureDefinition[]> {
    ...
    return (data || []).map((row: Record<string, unknown>) => ({
      ...
      subscriptionTier: row.subscription_tier as 'free' | 'growth' | 'pro',
      createdAt: new Date(row.created_at as string),
    }))
  },
}
