...
- Card: bank name dropdown (Myanmar banks), card number auto-detects type (Visa/Mastercard/Amex/Discover), last 4 digits stored
...
- Invoice counter is DB-owned and not user-editable from the Settings UI; invoice numbers come from the atomic database function `reserve_invoice_number(p_shop_id)` which mutates `app_settings.invoice_counter` atomically. Only an explicit admin reset workflow (if implemented) should modify the counter.
- Global Preferences section persists to `app_settings`: interface mode, theme, receipt printer toggle, auto-backup toggle, exchange-rate provider, exchange-rate API key, and exchange-rate update interval — **LEGACY (MMK-only v1):** exchange-rate fields are dead, multi-currency removed per VISION.md §19
...
- All steps succeed together or all roll back: sale creation, inventory deduction, customer stats
- **Server recomputes the total as `GREATEST(subtotal - discount + tax, 0)`; the client-supplied `total` is ignored and discount is clamped at `[0, subtotal]`**
- **Daily order limit (FR-LIMIT-01) is enforced inside the RPC — `DAILY_LIMIT_REACHED` on exceed**
...
| **Invoice Number** | `sales.invoice_number` | Formatted as `{prefix}-{counter}` (e.g., `INV-001001`). Generated inside `checkout_complete()` RPC via `reserve_invoice_number()`. |
